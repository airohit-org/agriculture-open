...
export default class AutoGraticule extends LayerGroup {
    ...
    constructor(options) {
        super();
        this.innerBounds = options.bounds;
        this.lineStyle = {
            stroke: true,
            color: '#111',
            opacity: 0.6,
    ...
    redraw() {
        this._bounds = this.innerBounds.pad(0.5) || this._map.getBounds().pad(0.5);
    ...
    constructLines() {
        const bounds = this.innerBounds || this._map.getBounds();
        ...
        try {
            ...
            for (let lng = AutoGraticule.fixFloatingPoint(Math.ceil(west/divisor)*divisor); lng <= east; lng+=divisor) {
                ...
                const gridlabelHoriz = this.buildLabel('gridlabel-horiz', AutoGraticule.fixFloatingPoint(lng))
                this.addLayer(gridlabelHoriz);
            }
            ...
            if(bounds.getNorth() > 0) {
                ...
                while(lat < bounds.getNorth() && lat < 85) {
                    ...
                    const gridlabelVert = this.buildLabel('gridlabel-vert', lat)
                    this.addLayer(gridlabelVert);
                }
            }
    ...
    buildLabel(axis, val) {
        const bounds = this.innerBounds.pad(-0.003) || this._map.getBounds().pad(-0.003);
        let latLng;
